...
const DB_NAME = "sqlite:vault.db";
let initPromise: Promise<Database> | null = null;

export const migrateFromLocalStorage = async (db: Database) => {
  try {
    ...
    for (const p of projects) {
      ...
      for (const m of p.members) {
        await db.execute(
          "INSERT OR REPLACE INTO members (id, project_id, name, role, total_spent, created_at) VALUES ($1, $2, $3, $4, $5, $6)", 
          [m.id, p.id, m.name, m.role, m.totalSpent, new Date().toISOString()]
        );
      }
      
      for (const l of p.logs) {
        await db.execute("INSERT OR REPLACE INTO logs (id, project_id, member_id, timestamp, type, context, value) VALUES ($1, $2, $3, $4, $5, $6, $7)", [l.id, p.id, l.memberId || null, l.timestamp, l.type, l.context, l.value]);
      }
    }
    
    await db.execute("COMMIT");
    localStorage.removeItem('vault-multi-project-storage');
    console.log("✓ Migration successful with Birth-Timestamp.");
  } catch (err) {
