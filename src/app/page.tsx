...
import { 
  ...
  AlertTriangle, Users, LayoutDashboard, X, FolderPlus, User, Trash2, RotateCcw, 
  FileText, Download, Fingerprint, ShieldCheck, CheckCircle2, Wifi, Loader2
} from 'lucide-react'
...
export default function VaultPage() {
  ...
  const handleVoidAction = async () => {
    if (isProcessing || !activeProject || !selectedLogId) return
    await deleteLog(activeProject.id, selectedLogId)
    handleCloseModal()
  }
  ...
  return (
    <div className="flex h-screen w-screen bg-matrix-bg font-mono text-white overflow-hidden select-none relative">
      ...
      <AnimatePresence>
        {downloadStatus && (
          <motion.div 
            ...
            exit={{ y: 50, opacity: 0 }}
            className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[500] flex items-center gap-3 bg-matrix-card border border-matrix-green p-4 shadow-glow-green"
          >
      ...
      <aside className="w-20 border-r border-matrix-border bg-matrix-card flex flex-col items-center py-8 gap-6 z-50 flex-shrink-0 shadow-glow-green/20">
        ...
        <div className="flex flex-col gap-6 overflow-y-auto no-scrollbar flex-1 w-full items-center mt-4">
            ...
            <div key={p.id} className="relative group flex items-center justify-center w-full px-2">
              {activeProjectId === p.id && (
                <motion.div 
                  ...
                  className="absolute left-0 w-1 h-8 bg-matrix-green shadow-glow-green" 
                />
              )}
              <motion.button 
                title={p.name} 
                ...
                className={cn(
                  ...
                  activeProjectId === p.id 
                    ...
                    : "border-matrix-border text-zinc-600"
                )}
              >
              ...
              <button 
                ...
                className="absolute -top-1 right-2 opacity-0 group-hover:opacity-100 bg-alert-pink text-white p-0.5 rounded-sm"
              >
                <X size={10} />
              </button>
            </div>
          ...
          <button 
            ...
            className="w-12 h-12 border border-dashed border-matrix-border text-zinc-700 hover:text-matrix-green hover:border-matrix-green transition-colors"
          >
            <Plus size={20} />
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col p-6 gap-6 min-w-0 overflow-hidden">
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            >
              ...
              <p className="text-[10px] text-matrix-green/40 font-black uppercase tracking-[0.5em] animate-pulse">
                Syncing_Vault_Database...
              </p>
            </motion.div>
          ) : activeProject ? (
            <motion.div 
            ...
            >
              ...
              <header className="flex justify-between items-center border-b border-matrix-border pb-4 flex-shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-sm font-black italic uppercase tracking-tighter text-matrix-green">
                      Vault // {activeProject.name}
                    </h1>
                    <p className="text-[8px] text-matrix-green/50 font-bold uppercase tracking-widest">
                ...
                <div className="flex gap-4 text-[9px] font-bold text-matrix-green items-center">
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1 border border-matrix-green/30 hover:border-matrix-green bg-matrix-green/5 transition-all text-matrix-green font-black"
                  >
                    <FileText size={12} /> EXPORT_LEDGER
                  </button>
                  <span className="opacity-40 uppercase tracking-tighter">
                    <Wifi size={14} className="inline mr-1" /> 12ms
                  </span>
                  <span className="px-3 py-1 border border-matrix-green bg-matrix-green/10 shadow-glow-green font-black uppercase">
                    Status: Secure
                  </span>
                </div>
              </header>
              ...
              <div className="flex-1 grid grid-cols-12 grid-rows-6 gap-6 overflow-hidden min-h-0">
                ...
                <section className="col-span-4 row-span-4 border border-matrix-border bg-matrix-card p-6 relative flex flex-col items-center justify-center overflow-hidden shadow-glow-green/10">
                  <div className="absolute top-3 left-3 text-[8px] text-matrix-green/40 uppercase font-black tracking-widest">
                    Core_Stability
                  </div>
                  <div className="relative w-full max-w-[220px] aspect-square flex items-center justify-center">
                    <motion.div 
                      ...
                      className="absolute inset-0 rounded-full border-[10px] border-matrix-border border-t-matrix-green shadow-glow-green"
                    />
                    <div className="flex flex-col items-center text-center">
                      <span className="text-[8px] opacity-40 uppercase font-black mb-1">Available_DP</span>
                      <span className="text-4xl font-black tracking-tighter text-matrix-green">
                        {(activeProject.balance ?? 0).toLocaleString()}
                      </span>
                      <span className="text-[10px] text-matrix-green font-bold uppercase mt-2 italic animate-pulse">
                        Sync_Active
                      </span>
                    </div>
                  </div>
                ...
                <section className="col-span-8 row-span-4 border border-matrix-border bg-matrix-card p-5 flex flex-col overflow-hidden">
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-4 overflow-hidden content-start pr-1">
                    {activeProject.members?.slice(0, 6).map((m) => {
                      const impact = (activeProject.balance + (m.totalSpent ?? 0)) > 0 
                        ? ((m.totalSpent ?? 0) / (activeProject.balance + (m.totalSpent ?? 0))) * 100 
                        : 0
                      return (
                        <motion.div 
                          ...
                          key={m.id} 
                          className="border border-matrix-border bg-matrix-card p-4 h-32 flex flex-col justify-between hover:border-matrix-green transition-all group cursor-pointer shadow-lg"
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex gap-3 min-w-0">
                              <div className="w-10 h-10 bg-zinc-900 border border-matrix-border flex items-center justify-center text-[9px] text-matrix-green/10 font-black">
                                HUD
                              </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="h-1 bg-matrix-border w-full overflow-hidden">
                              <motion.div 
                                ...
                                className="h-full bg-alert-pink shadow-glow-pink"
                              />
                            </div>
                          </div>
                        </motion.div>
                    ...
                    {activeProject.members?.length < 6 && (
                      <button 
                        title="Add Unit" 
                        onClick={() => setModalType("OPERATIVE")} 
                        className="border border-dashed border-matrix-border h-32 flex flex-col items-center justify-center gap-2 hover:border-matrix-green hover:bg-matrix-green/5 transition-all text-zinc-800"
                      >
                        <Plus size={20} />
                        <span className="text-[8px] font-black uppercase tracking-tighter">Reg_Unit</span>
                      </button>
                    )}
                  </div>
                </section>
                      ...
                      <tbody className="divide-y divide-matrix-border/30">
                        {activeProject.logs?.map((log) => (
                          <tr 
                            ...
                            className="hover:bg-[var(--hover-green)] transition-colors group cursor-pointer"
                          >
                            <td className="p-3 text-matrix-green/40 italic uppercase whitespace-nowrap px-4">
                              {new Date(log.timestamp).toLocaleTimeString('en-GB')}
                            </td>
                            <td className="p-3 px-1">
                              <span className={cn(
                                "px-2 py-0.5 border text-[7px] font-black uppercase",
                                log.type === 'EXPENSE' 
                                  ? "border-alert-pink text-alert-pink" 
                                  : "border-matrix-green text-matrix-green"
                              )}>
                                {log.type}
                              </span>
                            </td>
                            <td className="p-3 opacity-60 italic truncate max-w-[400px]">
                              "{log.context}"
                            </td>
                            <td className={cn(
                              "p-3 text-right font-black whitespace-nowrap px-4",
                              log.value < 0 ? "text-alert-pink" : "text-matrix-green"
                            )}>
                              {(log.value ?? 0).toLocaleString()} DP
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <section className="col-span-3 row-span-2 border border-matrix-border bg-matrix-card p-4 flex flex-col gap-3 justify-center shadow-glow-green/5">
                  <div className="text-[7px] font-black text-matrix-green/30 uppercase mb-1 tracking-[0.3em] text-center">
                    Execute_Command
                  </div>
                  <motion.button 
                    title="Inject DP" 
                    ...
                    className="flex justify-between items-center p-3 border border-matrix-green bg-matrix-green/5 text-matrix-green text-[10px] font-black hover:bg-matrix-green hover:text-black transition-all shadow-glow-green"
                  >
                  ...
                  <motion.button 
                    ...
                    className="flex justify-between items-center p-3 border border-alert-pink bg-alert-pink/5 text-alert-pink text-[10px] font-black hover:bg-alert-pink hover:text-white transition-all shadow-glow-pink"
                  >
            ...
            >
              ...
              <div className="space-y-2 mb-10">
                <h2 className="text-xl font-black italic uppercase tracking-[0.5em] text-matrix-green">
                  Vault_By_harikahono
                </h2>
                <p className="text-[10px] text-zinc-600 uppercase tracking-widest">
                  Terminal_Awaiting_Data_Input // Select_Vault_To_Begin
                </p>
              </div>
              <button 
                title="Init New Project" 
                onClick={() => setModalType("PROJECT")} 
                className="px-12 py-4 border border-matrix-green text-matrix-green text-[10px] font-black tracking-[0.5em] hover:bg-matrix-green hover:text-black transition-all shadow-glow-green"
              >
        ...
        {/* MODAL 1: UNIT AUDIT (Member Detail) */}
        <AnimatePresence>
          {selectedMemberId && detailMember && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-matrix-bg/95 backdrop-blur-lg">
              ...
              />
              <motion.div 
                initial={{ scale: 0.9, opacity: 0, skewX: 5 }} 
                animate={{ scale: 1, opacity: 1, skewX: 0 }} 
                exit={{ scale: 0.9, opacity: 0 }} 
                className="relative w-full max-w-2xl bg-matrix-card border border-matrix-border p-10 shadow-glow-green flex flex-col h-[80vh] overflow-hidden"
              >
                ...
                <div className="flex justify-between items-start mb-8 border-b border-matrix-border pb-6">
                  <div className="flex gap-6 items-center">
                    <div className="w-16 h-16 bg-zinc-900 border border-matrix-green flex items-center justify-center shadow-glow-green">
                      <User size={32} className="text-matrix-green" />
                    </div>
                    <div>
                      <h2 className="text-xl font-black uppercase text-matrix-green tracking-widest">
                        {detailMember.name}
                      </h2>
                      <p className="text-xs text-matrix-green/50 font-black uppercase italic tracking-[0.2em]">
                        {detailMember.role}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-black text-alert-pink">
                        {(detailMember.totalSpent ?? 0).toLocaleString()} DP
                      </p>
                    </div>
                    <button 
                      title="Download Audit" 
                      onClick={() => exportMemberPDF(detailMember)} 
                      className="p-2 border border-matrix-green/30 text-matrix-green/40 hover:text-matrix-green hover:border-matrix-green transition-all shadow-glow-green/50"
                    >
                      <Download size={18} />
                    </button>
                    {!isConfirmingDelete && (
                      <button 
                        ...
                        className="p-2 border border-alert-pink/30 text-alert-pink/40 hover:text-alert-pink hover:border-alert-pink transition-all"
                      >
                        <Trash2 size={18} />
                      </button>
                    )}
                    <button 
                      title="Close" 
                      onClick={handleCloseModal} 
                      className="ml-4 text-zinc-600 hover:text-alert-pink transition-colors"
                    >
                      <X size={24} />
                    </button>
                  </div>
                ...
                <AnimatePresence>
                  {isConfirmingDelete && (
                    <motion.div 
                      ...
                      className="mb-6 border border-alert-pink bg-alert-pink/5 p-6 overflow-hidden shadow-glow-pink/30"
                    >
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-alert-pink/60 uppercase">
                            Duit mau dibalikin atau dianggap hangus?
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-3">
                        <button 
                          ...
                          onClick={() => handleDecommission(true)} 
                          className="flex-1 py-3 bg-matrix-green/20 border border-matrix-green text-matrix-green text-[9px] font-black uppercase hover:bg-matrix-green transition-all flex items-center justify-center gap-2 shadow-glow-green/50"
                        >
                          {isProcessing ? <Loader2 className="animate-spin" size={12} /> : <><RotateCcw size={12} /> REFUND</>}
                        </button>
                        <button 
                          disabled={isProcessing} 
                          onClick={() => handleDecommission(false)} 
                          className="flex-1 py-3 bg-alert-pink/20 border border-alert-pink text-alert-pink text-[9px] font-black uppercase hover:bg-alert-pink transition-all flex items-center justify-center gap-2 shadow-glow-pink/50"
                        >
                    ...
                    <tbody className="divide-y divide-matrix-border/20">
                      {memberLogs?.map((log) => (
                        <tr key={log.id} className="hover:bg-[var(--hover-green)]">
                          <td className="py-3 text-matrix-green/30 italic whitespace-nowrap">
                            {new Date(log.timestamp).toLocaleTimeString('en-GB')}
                          </td>
                          <td className="py-3 px-4">
                            <span className={cn(
                              "px-2 py-0.5 border text-[8px] font-black uppercase",
                              !log.memberId ? "border-zinc-700 text-zinc-700" : "border-alert-pink text-alert-pink"
                            )}>
                              {!log.memberId ? "SHARED" : "DIRECT"}
                            </span>
                          </td>
                          <td className="py-3 opacity-80 uppercase tracking-tighter truncate max-w-[200px]">
                            "{log.context}"
                          </td>
                          <td className="py-3 text-right font-black text-alert-pink">
                            {(!log.memberId 
                              ? ((-log.value) / (activeProject?.members?.length || 1)) 
                              : -log.value
                            ).toLocaleString()} DP
                          </td>
                        </tr>
          ...
          {selectedLogId && detailLog && (
            <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-matrix-bg/95 backdrop-blur-lg">
              ...
              <motion.div 
                ...
                className="relative w-full max-w-md bg-matrix-card border border-matrix-green/50 p-8 shadow-glow-green"
              >
                <div className="flex justify-between items-center mb-8 border-b border-matrix-border pb-4">
                  <h2 className="text-xs font-black uppercase text-matrix-green tracking-[0.4em] flex items-center gap-3">
                    <ShieldCheck size={16} /> Transaction_Audit
                  </h2>
                  <div className="flex items-center gap-3">
                    {!isConfirmingVoid && (
                      <button 
                        onClick={() => setIsConfirmingVoid(true)} 
                        className="text-alert-pink hover:text-white transition-colors p-1 border border-alert-pink/30 hover:border-alert-pink shadow-glow-pink/30"
                      >
                        <Trash2 size={16} />
                      </button>
                    )}
                    <button 
                      title="Close" 
                      onClick={handleCloseModal} 
                      className="text-zinc-600 hover:text-alert-pink"
                    >
                      <X size={20} />
                    </button>
                  </div>
                ...
                <AnimatePresence>
                  {isConfirmingVoid && (
                    <motion.div 
                      ...
                      animate={{ height: "auto", opacity: 1 }} 
                      exit={{ height: 0, opacity: 0 }} 
                      className="mb-6 p-4 border border-alert-pink bg-alert-pink/10 text-[10px] space-y-4 shadow-glow-pink/20"
                    >
                      <p className="font-black uppercase text-alert-pink tracking-widest animate-pulse">
                        Void_Sequence: Hapus transaksi ini dan koreksi saldo otomatis?
                      </p>
                      <div className="flex gap-2">
                        <button 
                          disabled={isProcessing} 
                          onClick={handleVoidAction} 
                          className="flex-1 py-2 bg-alert-pink text-white font-black uppercase hover:bg-white hover:text-alert-pink transition-all shadow-glow-pink"
                        >
                          {isProcessing ? <Loader2 className="animate-spin inline mr-2" size={12} /> : "Confirm_Void"}
                        </button>
                        <button 
                          onClick={() => setIsConfirmingVoid(false)} 
                          className="px-4 py-2 border border-zinc-700 text-zinc-500 font-black uppercase hover:bg-zinc-800 transition-all"
                        >
                          Abort
                        </button>
                      </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-matrix-bg border border-matrix-border">
                      <p className="text-[7px] text-zinc-600 uppercase font-black mb-1">Auth_Protocol</p>
                      <p className="text-[10px] font-black text-matrix-green">{detailLog.type}</p>
                    </div>
                    <div className="p-3 bg-matrix-bg border border-matrix-border">
                      <p className="text-[7px] text-zinc-600 uppercase font-black mb-1">Time_Sync</p>
                      <p className="text-[10px] font-black text-white">
                        {new Date(detailLog.timestamp).toLocaleTimeString('en-GB')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border border-matrix-border bg-matrix-green/5">
                    ...
                    <p className={cn(
                      ...
                      detailLog.value < 0 ? "text-alert-pink" : "text-matrix-green"
                    )}>
                      {detailLog.value.toLocaleString()} DP
                    </p>
                    <div className="mt-4 pt-4 border-t border-matrix-border flex justify-between items-center">
                      ...
                      <span className="text-[9px] font-black text-matrix-green uppercase">
                        {detailLog.memberId 
                          ? activeProject?.members.find(m => m.id === detailLog.memberId)?.name 
                          : "SHARED_ALLOCATION"}
                      </span>
                    </div>
                  </div>
        ...
        {/* MODAL 3: INPUT SEQUENCES (PROJECT / OPERATIVE / EXPENSE / INJECTION) */}
        <AnimatePresence>
          {modalType && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-matrix-bg/95 backdrop-blur-lg">
              ...
              <motion.div 
                ...
                animate={{ scale: 1, opacity: 1, skewX: 0 }} 
                exit={{ scale: 0.9, opacity: 0 }} 
                className="relative w-full max-w-sm bg-matrix-card border border-matrix-border p-10 shadow-glow-green overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-matrix-green to-transparent opacity-50" />
                
                <h2 className="text-[12px] font-black uppercase text-matrix-green mb-8 border-b border-matrix-border pb-4 tracking-[0.4em] text-center">
                  {modalType}_SEQUENCE
                </h2>
                ...
                <div className="space-y-6">
                  {modalType === "EXPENSE" && (
                    <div className="space-y-2">
                      ...
                      <select 
                        title="Select Unit" 
                        className="w-full bg-matrix-bg border border-matrix-border p-4 text-[10px] text-matrix-green outline-none appearance-none cursor-pointer focus:border-matrix-green font-bold"
                  ...
                  <input 
                    autoFocus 
                    className="w-full bg-matrix-bg border border-matrix-border p-4 text-[10px] text-matrix-green outline-none focus:border-matrix-green font-bold uppercase" 
                  ...
                  {(modalType === "EXPENSE" || modalType === "INJECTION") && (
                    <input 
                      type="number" 
                      className={cn(
                        "w-full bg-matrix-bg border border-matrix-border p-4 text-[10px] outline-none font-bold",
                        modalType === "EXPENSE" 
                          ? "text-alert-pink focus:border-alert-pink" 
                          : "text-matrix-green focus:border-matrix-green"
                      )} 
                      placeholder="DP_Magnitude" 
                  ...
                  {modalType === "OPERATIVE" && (
                    <input 
                      className="w-full bg-matrix-bg border border-matrix-border p-4 text-[10px] text-white outline-none focus:border-white uppercase font-bold" 
                  ...
                  <div className="flex gap-4 pt-4">
                    <button 
                      ...
                      className={cn(
                        "flex-1 py-4 text-[11px] font-black uppercase tracking-[0.2em] transition-all shadow-lg flex items-center justify-center gap-2",
                        modalType === "EXPENSE" 
                          ? "bg-alert-pink/20 border border-alert-pink text-alert-pink hover:bg-alert-pink hover:text-white shadow-glow-pink" 
                          : "bg-matrix-green/20 border border-matrix-green text-matrix-green hover:bg-matrix-green hover:text-black shadow-glow-green"
                      )}
                    >
                    ...
                    <button 
                      disabled={isProcessing} 
                      onClick={handleCloseModal} 
                      className="px-6 py-4 border border-zinc-800 text-zinc-600 text-[10px] font-black hover:bg-zinc-800 transition-all uppercase"
                    >
                      Abort
                    </button>
                  </div>
